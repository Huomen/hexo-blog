...
**注意：**
不需要下载`activation.jar`，JDK1.5以后这个jar包已经集成到JDK了，亲测有效。
		...
		try {
			...
			Session session = Session.getDefaultInstance(properties, new Authenticator() {
				...
				public PasswordAuthentication getPasswordAuthentication() {
					return new PasswordAuthentication("husen@hemingsheng.cn", "此处填密码（QQ邮箱填授权码）");
				}
			});
			...
			// 发件人邮箱号和密码 与前面填密码选一处即可
			// transport.connect("940706904@qq.com", "此处填密码");
			transport.connect();
			// 对方的地址
			transport.sendMessage(message, new Address[] { new InternetAddress("对方的邮箱@qq.com") });
			// 关闭连接
			transport.close();
